# segunda_especie/reglas.py (v8 - Análisis de Figuras Disonantes)
...
def verificar_inicio_final_segunda_especie_modificado(cf_part, cp_part):
    ...
    # ... (Esta función no ha cambiado y se mantiene como estaba) ...
    errores = []
    cf_flat_notes = list(cf_part.flatten().notes)
    cp_flat_elements = list(cp_part.flatten().notesAndRests) 
    if not cf_flat_notes: errores.append("Regla Inicio/Final: Cantus Firmus no contiene notas.")
    if not cp_flat_elements: errores.append("Regla Inicio/Final: Contrapunto no contiene elementos.")
    ...
    for el_idx in range(len(cp_flat_elements) - 1, -1, -1):
        ...
        if isinstance(el, note.Note):
            ...
            elif cp_penultima_nota_cp is None and el_idx < len(cp_flat_elements) -1 : cp_penultima_nota_cp = el; break
...
def _analizar_figura_de_suspension(cp_nota_prep, cp_nota_sus, cp_nota_res, cf_part_obj):
    """
    Función auxiliar que analiza una figura de 3 notas (prep, sus, res)
    y devuelve los errores, observaciones y notas para colorear.
    """
    errores = []
    observaciones = []
    ids_rojos = []
    ...
    try:
        ...
        if es_disonancia(cf_nota_prep, cp_nota_prep):
            msg = f"Compás {m_sus.number-1}: Suspensión mal preparada. El intervalo de preparación '{intervalo_prep.niceName}' es disonante."
            errores.append(msg)
        ...
        if intervalo_sus.simpleName not in ['m2', 'M2', 'P4', 'm7', 'M7', 'm9', 'M9']:
            msg = f"Compás {m_sus.number}: La nota {cp_nota_sus.nameWithOctave} forma un intervalo de '{intervalo_sus.niceName}', que no es una disonancia de suspensión/retardo típica (2, 4, 7, 9)."
        ...
        if es_consonancia(cf_nota_sus_res, cp_nota_res) and movimiento_resolucion.isStep and movimiento_resolucion.direction < 0:
            ...
            msg = f"Compás {m_sus.number}: Suspensión {intervalo_sus.simpleName}-{intervalo_res.simpleName} resuelta correctamente."
            observaciones.append(msg)
        
        if not es_resolucion_correcta:
            if es_consonancia(cf_nota_sus_res, cp_nota_res) and movimiento_resolucion.isStep and movimiento_resolucion.direction > 0:
                msg = f"Error en Compás {m_sus.number}: La figura {intervalo_sus.simpleName}-{intervalo_res.simpleName} es un 'Retardo' (resolución ascendente). En el contrapunto estricto de especies, la resolución de una suspensión debe ser descendente."
                errores.append(msg)
            elif es_disonancia(cf_nota_sus_res, cp_nota_res):
                msg = f"Error en Compás {m_sus.number}: La suspensión {intervalo_sus.simpleName} resuelve a una disonancia ({intervalo_res.niceName})."
                errores.append(msg)
            elif not movimiento_resolucion.isStep:
                msg = f"Error en Compás {m_sus.number}: La suspensión {intervalo_sus.simpleName} no resuelve por grado conjunto (resolvió con un salto de {movimiento_resolucion.niceName})."
                errores.append(msg)
            else:
                msg = f"Error en Compás {m_sus.number}: La suspensión {intervalo_sus.simpleName}-{intervalo_res.simpleName} tiene una resolución irregular no clasificada."
                errores.append(msg)
            
            ids_rojos.extend([cp_nota_sus.id, cp_nota_res.id])

    except Exception as e:
        errores.append(f"Error interno analizando figura en compás {m_sus.number}: {e}")
...
def analizar_figuras_disonantes_2da_especie(cp_part_obj, cf_part_obj):
    ...
    Busca suspensiones (ligadas o por acento) y en el futuro, notas de paso y bordaduras.
    """
    todos_errores = []
    todas_observaciones = []
    todos_ids_rojos = []
    
    cp_notes_flat = list(cp_part_obj.flatten().notes)
    notas_ya_analizadas = set() # Para no analizar una nota dos veces
        ...
        if cp_nota_actual.beat == 1.0:
            ...
            if es_disonancia(cf_nota_simultanea, cp_nota_actual):
                ...
                if i > 0 and (i + 1) < len(cp_notes_flat):
                    ...
                    if not (cp_nota_prep.tie and cp_nota_prep.tie.type == 'start'):
                        todas_observaciones.append(f"Compás {cp_nota_actual.getContextByClass(stream.Measure).number}: Disonancia de tiempo fuerte ('{interval.Interval(cf_nota_simultanea, cp_nota_actual).niceName}') sin ligadura de preparación (suspensión por acento).")
